...
const AbsentList = () => {
  ...
  const fetchAbsents = async () => {
    try {
      const res = await axios.get('http://localhost:8080/api/abs/get/all');
      setAbsents(res.data);
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingId) {
        await axios.put('http://localhost:8080/api/abs/absent/', formData);
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete('http://localhost:8080/api/abs/absent/${id}');
